...
def load_return_period_bnds(
    data, model_class, save_fp, bounds, n_samples=1000, alpha=0.05, MC=False, **kwargs
):
    ...
    if os.path.isfile(save_fp):
        ...
        if not MC:
            ...
            mean, median, lb, ub = compute_MC_return_period_bnds(
                data, model_class=model_class, bounds=bounds, alpha=alpha, **kwargs
            )
            ## save to file
            with open(save_fp, "wb") as file:
...
def compute_MC_return_period_bnds(
    full_data, model_class, bounds, alpha=0.05, n_train=80, n_mc=10
):
    ...
    # return mean, median, lb, ub
    return (
        np.mean(return_levels_samples, axis=0),
        np.median(return_levels_samples, axis=0),
        lb,
        ub,
    )
...
def get_empirical_pdf(data, bin_edges=None):
    ...
    if bin_edges is None:
        print(
            f"Number of bins not specified. Using square root rule. {np.sqrt(len(data)).astype(int)}"
        )
        bin_edges = np.linspace(
            np.floor(data.min()), np.ceil(data.max()), np.sqrt(len(data)).astype(int)
        )
...
def compute_MC_return_value(
    full_data, model_class, bounds, n_train=80, n_mc=10, return_periods=100
):
    ...
    for i in tqdm(range(n_mc)):
        ...
        if model_class == scipy.stats.norm:
            ...
            model = fit_model(train_data, model_class=model_class, bounds=bounds)
        # compute return period
        return_levels_samples.append(
            get_return_levels(model, return_periods=return_periods)[0]
        )
